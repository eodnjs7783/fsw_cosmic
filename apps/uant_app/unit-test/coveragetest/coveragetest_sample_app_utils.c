...
void Test_UANT_APP_TblValidationFunc(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(UANT_APP_TblValidationFunc(&TestTblData), CFE_SUCCESS);

    /* error case should return UANT_APP_TABLE_OUT_OF_RANGE_ERR_CODE */
    TestTblData.Int1 = 1 + UANT_APP_TBL_ELEMENT_1_MAX;
    UtAssert_INT32_EQ(UANT_APP_TblValidationFunc(&TestTblData), UANT_APP_TABLE_OUT_OF_RANGE_ERR_CODE);
}

void Test_UANT_APP_GetCrc(void)
{
    /*
     * Test Case For:
...
{
    ADD_TEST(UANT_APP_TblValidationFunc);
    ADD_TEST(UANT_APP_GetCrc);
}
